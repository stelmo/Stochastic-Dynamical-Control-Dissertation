...
\caption{The ellipse centred around $\mu_1$ intersects the constraint while the ellipse centred around $\mu_2$ is wholly above (satisfies) the constraint.}
...
It is useful to introduce the measures we will use to quantify the effectiveness of the control techniques used in this and the following chapters. We define the average energy input by $\frac{h}{N}\sum^N_{t=0}|u_t-u_s|$ and the average concentration error by $\frac{1}{N}\sum^N_{t=0}|\frac{{C_A}_t-y_{sp}}{y_{sp}}|$.
...
\begin{figure}[H]
\centering
\includegraphics[scale=0.25]{lin_mod_kf_mean_mc.pdf}
...
\begin{figure}[H]
\centering
\includegraphics[scale=0.25]{lin_mod_kf_var_conf_90_mc.pdf}
\caption{Cumulative probability distribution of the shortest Mahalanobis distance of each state to the constraint based on 1000 simulations. Negative distances imply the constraint was violated.}
...
In this section we consider the problem of controlling the full non-linear system with a linear model linearised around the unsteady operating point. The control goal is the same as before; the only difference between this section and Chapter \ref{sec_lin_sys_cont} is that the underlying plant is non-linear.
...
\begin{figure}[H] 
\centering
\includegraphics[scale=0.25]{nonlin_mod_kf_mean_track.pdf}
