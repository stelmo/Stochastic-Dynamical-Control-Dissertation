...
\begin{thrm}
\textbf{Affine Expected Value Constraints} Suppose
\end{thrm}

\begin{thrm}
\textbf{Shortest Mahalanobis Distance between a Hyperplane and a Point} Suppose we are given a positive semi-definite matrix $S$ and a point $y$. The shortest Mahalanobis Distance between $y$ and the hyperplane $b^Tx+c=0$ is given by $\frac{|b^Ty+c|}{\sqrt{b^TSb}}$.
\label{thrm_mahala_dist}
\end{thrm}
\begin{proof}
It is natural to formulate Theorem \ref{thrm_mahala_dist} as an optimisation problem as shown in (\ref{eq_mahala_optim}). Note that we have squared the Mahalanobis distance function to simplify the analysis.
\begin{equation}
\begin{aligned}
...
\end{equation}
Recalling that the objective function was squared we can then conclude that the shortest Mahalanobis Distance between a point $y$ and the constraints of (\ref{eq_mahala_optim}) is $\frac{\sqrt{(b^Ty+c)^2}}{\sqrt{b^TSb}} = \frac{|b^Ty+c|}{\sqrt{b^TSb}}$.
\end{proof}

\begin{thrm}
\textbf{Affine Chance Constraints}
\end{thrm}
