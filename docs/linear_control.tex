...
\end{equation}
The tuning parameters used in this and all subsequent sections are shown in (\ref{eq_mpc_tuning}). Note that the magnitude difference between the components of $Q$ and $R$ is necessary because the units of concentration and heat input are not scaled. Additionally, we assume a zero order hold of 1 min between controller inputs.
\begin{equation}
\begin{aligned}
Q = \begin{pmatrix}
1\times 10^{4} & 0 \\ 0 & 0
...
\label{eq_mpc_tuning}
\end{equation}
Firstly we illustrate the approach of using only the result of Theorem \ref{thrm_lqg_sol} i.e. we apply the LQG regulator to the system. Figure shows that the system does indeed converge, noisily, to the set point. The primary drawback of this method is that there is no easy way to constrain the system. From a practical perspective this can be problematic. 
\begin{figure}[H] 
\centering
...
& \text{subject to } \mu_{t+1}=A\mu_t + Bu_t \\
& d^T\mu_t + e \geq 0 ~\forall ~t=1,...,N\\
&  |u_t| \leq 11000 ~\forall ~t=0,...,N-1\\
&  |u_t-u_t{-1}| \leq 1000 ~\forall ~t=1,...,N-1
\end{aligned}
\label{eq_mpc_constrained_det1}
\end{equation}
We only use a single state constraint in this dissertation but the extension to multiple constraints is straightforward. We have that $d^T = (10, 1)$ and $e=(412)$. The prediction and control horizon is equal and set at 150 time steps i.e. 15 minutes into the future. We additionally constrain the magnitude and rate of change of the inputs.

We assume that a Kalman Filter supplies the initial state estimate $x_0$ and that the mean $\mu_0$ of that estimate is used for control. Since we have assumed normality the deterministic inequality constraint can also be seen as an affine expected value constraint as shown in Theorem \ref{thrm_affine_expected_const}.

In Figure \ref{fig_lin_mod_kf_mean_track} we see the reference tracking and controller input for the deterministic MPC as shown in (\ref{eq_mpc_constrained_det1}).
\begin{figure}[H] 
\centering
...
\end{figure}
It is clear that we have noisy convergence to the set point. As mentioned previously we will never be able to achieve zero set point offset because of the noise term in the system dynamics (\ref{eq_lin_system}). 

\begin{figure}[H] 
\centering
\includegraphics[scale=0.25]{lin_mod_kf_mean_ss.pdf}
\caption{Deterministic MPC tracking with initial condition $(0.55, 450)$ and measuring both states.}
