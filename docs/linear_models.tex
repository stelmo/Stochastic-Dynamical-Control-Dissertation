...
In this section we apply the Kalman Filter to the CSTR introduced earlier. We use the linear model around the unstable operating point $(C_A, T_R)_0^2$ as shown explained in \textbf{Section 4} and shown in (\ref{eq_linmodel}) for convenience. Note that the matrix $A$ and vectors $B, b$ depend on the step size and should be recalculated for each different $h$. To make things concrete we have used $h=0.01$ here.
\begin{equation}
\begin{aligned}
&\begin{pmatrix}
C_A \\ T_R
...
\label{eq_linmodel}
\end{equation}
By applying the Kalman Filter we have
