...
\end{equation}
%We know that $x_0=\mu_0$ by assumption. Referring back to Section \ref{sec_lin_prediction} we also know that given the current state estimate the optimal $h$ step ahead prediction is given by $&\text{with } \mu_{t+h|t} = A\mu_{t+h-1|t}+Bu_{t+h-1} \\
%&\text{and } \Sigma_{t+h|t} = W+A\Sigma_{t+h-1|t} A^T \\
%&\text{and } \mu_{t+1|t} = A\mu_t+Bu_{t} \\
%&\text{and } \Sigma_{t+1|t} = W+A\Sigma_t A^T \\ $
\end{proof}
