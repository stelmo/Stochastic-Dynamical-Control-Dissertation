...
\label{eq_linmodel}
\end{equation}
We set the system noise $Q = \begin{pmatrix}
1\times 10^{-6} & 0 \\ 0 & 4.0 
\end{pmatrix}$ to indicate that the standard deviation of the concentration component of the model is 0.001 $kmol/m^{-3}$ and the temperature component is 2 $K$. This indicates that we are quite uncertain about the accuracy of the linear model. We set the measurement noise at $R = 2.0$ implying that 68\% of the measurements will fall between $\pm\sqrt{2}$ of the actual state. We use a vague initial state with mean at the initial condition and covariance $Q$.
