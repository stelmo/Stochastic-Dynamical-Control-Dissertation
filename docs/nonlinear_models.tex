...
Therefore, while the Particle Filter may be computationally more expensive to use it is a better filter if the system exhibits nonlinearity or non-Gaussian distributions. But, if the system is linear one is better off using the standard Kalman Filter.